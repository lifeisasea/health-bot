...
BASE_PERSONA = (
    ...
    "СОВЕТЫ ПО ЕДЕ ДЕЛАЙ РАЗНООБРАЗНЫМИ: смотри в тренды за 2 недели, что пользователь "
    "и так ест часто, и предлагай то, чего давно НЕ было; черпай идеи из разных кухонь "
    "(средиземноморская, ближневосточная, азиатская, кавказская…); не повторяй из раза "
    "в раз одни и те же дежурные блюда (лосось-гречка-овощи) — у каждой рекомендации "
    "давай 2–3 непохожих варианта на выбор. Пользователь живёт в Дубае — продукты "
    "любых кухонь доступны. "
    + DISCLAIMER
)
...
def food_logging_prompt() -> str:
    ...
    return (
        ...
        + "\n\n"
        + _context_block()
        + "\nЗАДАЧА: сначала определи, ЧТО на фото:\n"
        "- \"food\" — еда или напиток;\n"
        "- \"lab\" — бланк/результат анализов, медицинский документ, таблица показателей;\n"
        "- \"other\" — что-то иное (не еда и не анализ).\n"
        "Если food — оцени РАЗМЕР ПОРЦИИ и пищевую ценность так:\n"
        "1) Найди в кадре ОПОРНЫЕ ОБЪЕКТЫ известного размера: рука/ладонь (ширина ладони "
        "взрослой женщины ~8 см, длина кисти ~17 см, большой палец ~5–6 см), вилка/ложка "
        "(~19–20 см), нож, стандартный стакан (~250 мл), кружка (~300–350 мл), банка (330 мл), "
        "телефон (~15 см), банковская карта (8.6 см).\n"
        "2) По ним откалибруй диаметр тарелки/миски (обеденная ~26–28 см, десертная ~20–22 см, "
        "миска ~15–18 см) и оцени объём/вес каждой позиции с учётом высоты горки еды.\n"
        "3) Если опорных объектов нет — используй типичные размеры посуды и снизь confidence.\n"
        "Оценки приблизительные — это нормально, но будь последовательна: одинаковые продукты "
        "оценивай одинаково. Если lab или other — поля КБЖУ оставь нулями.\n"
        "Ответь ТОЛЬКО валидным JSON без пояснений в формате:\n"
        '{"type": "food|lab|other", '
        '"description": "краткое название блюда и порции (для food)", '
        '"items": ["ингредиент1", "ингредиент2"], '
        '"portion_g": суммарный_вес_еды_в_граммах, '
        '"scale_ref": "рука|приборы|посуда|стакан|нет", '
...
def router_prompt() -> str:
    ...
    return (
        ...
        "пересчитай КБЖУ. ВАЖНО: если правка меняет только состав/название, а не размер порции — "
        "калорийность должна остаться примерно прежней (±15%), не занижай и не завышай её резко. "
        "Если непонятно, к какой записи относится правка — НЕ угадывай, попроси уточнить.\n"
        "ЕСЛИ ПОЛЬЗОВАТЕЛЬ ОТВЕЧАЕТ ЦИТАТОЙ на сообщение бота о записанном блюде (начинается с 📝 или "
        "содержит описание блюда с ккал) — его текст почти всегда УТОЧНЕНИЕ/ИСПРАВЛЕНИЕ ИМЕННО ТОЙ записи: "
        "используй correct_meal (найди её в списке по описанию из цитаты), НЕ создавай новую запись add_meal.\n"
        "Если менять профиль/еду не нужно — actions: []. Не выдумывай состояния без явных слов пользователя.\n"
        "НИКОГДА не называй в reply итог калорий за день — точный итог из дневника добавится "
        "к ответу автоматически после применения действий. Не складывай калории в уме.\n"
